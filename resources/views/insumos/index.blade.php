        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Total de Insumos</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            ...
            <form method="GET" action="{{ route('insumos.index') }}" id="filter-form">
                ...
                <select name="estoque"
                    ...
                    <option value="baixo" @selected(request('estoque') === 'baixo')>⚠️ Estoque Baixo</option>
                    <option value="ok" @selected(request('estoque') === 'ok')>✅ Estoque OK</option>
                </select>
                ...
                @if(request()->hasAny(['search', 'categoria', 'estoque', 'sort']))
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-900">
                        <tr>
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-gray-900 dark:text-white uppercase tracking-wider">Nome</th>
                            <th class="px-4 py-3.5 text-center text-xs font-semibold text-gray-900 dark:text-white uppercase tracking-wider">Categoria</th>
                            <th class="px-4 py-3.5 text-center text-xs font-semibold text-gray-900 dark:text-white uppercase tracking-wider">Unidade</th>
                            <th class="px-4 py-3.5 text-center text-xs font-semibold text-gray-900 dark:text-white uppercase tracking-wider">Custo Unitário</th>
                            <th class="px-4 py-3.5 text-center text-xs font-semibold text-gray-900 dark:text-white uppercase tracking-wider">Estoque Atual</th>
                            <th class="px-4 py-3.5 text-center text-xs font-semibold text-gray-900 dark:text-white uppercase tracking-wider">Ponto Mínimo</th>
                            <th class="px-4 py-3.5 text-center text-xs font-semibold text-gray-900 dark:text-white uppercase tracking-wider">Status</th>
                            <th class="px-4 py-3.5 text-right text-xs font-semibold text-gray-900 dark:text-white uppercase tracking-wider min-w-[200px]">Ações</th>
                        </tr>
                    </thead>
                        ...
                        @forelse ($insumos as $insumo)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center text-gray-700 dark:text-gray-300 font-bold text-sm">
                                            {{ strtoupper(substr($insumo->nome, 0, 2)) }}
                                        </div>
                                        <div>
                                            <p class="font-semibold text-gray-900 dark:text-white">{{ $insumo->nome }}</p>
                                            @if($insumo->descricao)
                                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ Str::limit($insumo->descricao, 40) }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-4 text-center">
                                    @if($insumo->categoria)
                                        <span class="inline-flex items-center justify-center px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
                                            {{ $insumo->categoria }}
                                        </span>
                                    @else
                                ...
                                <td class="px-4 py-4 text-center">
                                    ...
                                    @else
                                        <span class="text-xs text-gray-400 dark:text-gray-500 italic">Sem estoque</span>
                                    @endif
                                </td>
                                <td class="px-4 py-4 text-sm text-gray-600 dark:text-gray-400 text-center">
                                    {{ $pontoMinimo > 0 ? number_format($pontoMinimo, 2, ',', '.') : '—' }}
                                </td>
                                <td class="px-4 py-4 text-center">
                                    @if(!$insumo->estoque)
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
                                            Sem estoque
                                        </span>
                                    @elseif($estoqueBaixo)
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-2 flex-wrap">
                                        <a href="{{ route('insumos.edit', $insumo) }}"
                                           class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                                        ...
                                        @if($insumo->estoque)
                                            <a href="{{ route('estoque.edit', $insumo->estoque) }}"
                                               class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-blue-700 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors"
                                        ...
                                        @else
                                            <a href="{{ route('estoque.create') }}?insumo_id={{ $insumo->id }}"
                                               class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
                                        ...
                                        <form action="{{ route('insumos.destroy', $insumo) }}" method="POST" data-confirm="Deseja remover este insumo?">
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-16 text-center">
                                    ...
                                    <p class="text-gray-500 dark:text-gray-400 font-medium">Nenhum insumo encontrado.</p>
                                    <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">Tente ajustar os filtros ou adicione um novo insumo.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
